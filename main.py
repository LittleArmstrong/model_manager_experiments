...
def create_augmented_dataset(original_dataset_dir: str, output_dir: str, transform: A.BasicTransform, aug_name:str):
    ...
    if (output_dir / "images" / "train").exists() and (output_dir / "labels" / "train").exists():
        ...
        images_train_dir = output_dir / "train" / "images"
        labels_train_dir = output_dir / "train" / "labels"
    else:
    ...
    for img_path in images_train_dir.glob("*.*"):
        if pattern.search(img_path.name):
            ...
            # Label laden
            label_path = labels_train_dir / f"{base_name}.txt"
            ...
            if aug_name == "copy_paste":
                ...
                img_new, boxes_new, class_labels_new = copy_paste_objects(
                    img, boxes, class_labels, labels_train_dir, images_train_dir, object_count_max=object_count_max
                )
            ...
            # Bild speichern (überschreibt das alte)
            cv2.imwrite(str(img_path), transformed["image"])
